...
for i in $@; do 
	...
	if [ "$2" = "" ] ; then
		...
		if [ "$1" = "current_pw" ] ; then
			curr_pw=`urldecode $2`
			hash=`echo -n "admin:antMiner Configuration:$curr_pw" | md5sum | cut -b -32`
			echo "admin:antMiner Configuration:$hash" > /tmp/validate_pw.tmp.$$
...
if [ "$error" = "false" ] ; then
	...
	hash=`echo -n "admin:antMiner Configuration:$new_pw" | md5sum | cut -b -32`
	echo "admin:antMiner Configuration:$hash" > \
